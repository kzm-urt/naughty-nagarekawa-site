...
(function () {
  ...
  const PUBLIC_ONLY = document.currentScript?.dataset.publicOnly === "true";
  ...
  function readData() {
    ...
    const remote = clone(window.NAUGHTY_REMOTE_DATA || null);
    if (remote && typeof remote === "object" && Object.keys(remote).length) {
      return normalizeRaw({
        ...fallback,
        ...remote,
        shop: { ...(fallback.shop || {}), ...(remote.shop || {}) },
        staff: Array.isArray(remote.staff) ? remote.staff : fallback.staff,
        shifts: Array.isArray(remote.shifts) ? remote.shifts : fallback.shifts,
        events: Array.isArray(remote.events) ? remote.events : fallback.events,
        products: Array.isArray(remote.products) ? remote.products : fallback.products,
        materials: Array.isArray(remote.materials) ? remote.materials : fallback.materials
      });
    }
    if (PUBLIC_ONLY) return normalizeRaw(fallback);
    let saved = null;
    try {
  ...
  function mapEmbedUrl(address) {
    return `https://www.google.com/maps?q=${encodeURIComponent(address || "広島市中区流川町")}&output=embed`;
  }
  ...
  function buildCast(data, staffList, days) {
    return staffList.map((staff, index) => {
      ...
      const real = asset(staff.heroRealPhoto || staff.photo || staff.portraitIcon, img);
      ...
      return {
        ...
        img,
        real,
        card: asset(staff.photo || staff.heroRealPhoto || staff.portraitIcon, real),
  ...
  window.NTY = buildNty();
  window.NTYRefresh = (remoteData) => {
    if (remoteData) window.NAUGHTY_REMOTE_DATA = clone(remoteData);
    window.NTY_SOURCE_DATA = readData();
    window.NTY = buildNty();
    return window.NTY;
  };
